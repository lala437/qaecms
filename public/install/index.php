...
$html_footer = <<<EOF
<div class="footer">
  <h5>Powered by <font class="blue">快简易CMS</font><font class="orange"></font></h5>
  <h6>版权所有 2020-2020 &copy; <a href="http://www.qaecms.com" target="_blank">快简易CMS</a></h6>
</div>
EOF;
...
//    }
    ...
    $sql = file_get_contents("data/qaecms.sql");
    $sql .= file_get_contents("data/qaecms_add.sql");
    $sql = str_replace("\r\n", "\n", $sql);
    runquery($sql, $mysqli);
    ...
    //管理员账号密码
    $res = $mysqli->query("INSERT INTO `qaecms_admins` VALUES (1, '{$username}','" . md5($password) . "','xxxxx@qq.com', NULL, NULL,NULL, NULL, NULL, NULL, NULL, '" . date('Y-m-d H:i:s', time()) . "', '" . date('Y-m-d H:i:s', time()) . "');");
    //新增一个标识文件，用来屏蔽重新安装
    $fp = @fopen('lock', 'wb+');
    @fclose($fp);
    exit("<script type=\"text/javascript\">document.getElementById('install_process').innerHTML = '安装完成，下一步...';document.getElementById('install_process').href='index.php?step=4';</script>");
    exit();
}

//execute sql
function runquery($sql, $mysqli)
{
...
function write_config()
{
    ...
    $config = ROOT_PATH . '/../config/mysql.php';
    @copy(ROOT_PATH."/install/config/mysql.php",$config);
